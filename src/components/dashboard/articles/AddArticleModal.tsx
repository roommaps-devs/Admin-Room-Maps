...
import { uploadRequest, postRequest } from "@/lib/apiCall";
...
export default function AddArticleModal({
  ...
  // Set default category when categories prop is loaded
  useEffect(() => {
    if (categories && categories.length > 0 && !selectedCategoryId) {
      const firstCategoryName = categories[0];
      const firstCategoryId = categoryMap[firstCategoryName.toLowerCase()] || firstCategoryName;
      setValue("categoryId", firstCategoryId);
    }
  }, [categories, selectedCategoryId, setValue, categoryMap]);
  ...
  const onSubmit = async (dt: ArticleFormValues) => {
    ...
    try {
      ...
      // 3. Make POST request to /admin/article/add
      const res = await postRequest<ApiResponse>("/admin/article/add", { data });
